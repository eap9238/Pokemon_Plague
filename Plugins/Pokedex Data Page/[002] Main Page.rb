...
class PokemonPokedexInfo_Scene 
  ...
  def pbGenerateDataLists(species)
    ...
    if species.mega_stone
      @data_hash[:item][0] = [species.mega_stone]
    elsif species.form_name == _INTL("Primal {1}", species.name)
      case species.species
      when :GROUDON then @data_hash[:item][0] = [:REDORB]
      when :KYOGRE  then @data_hash[:item][0] = [:BLUEORB]
      end
    elsif PluginManager.installed?("ZUD Mechanics") && species.ultra_item
      @data_hash[:item][0] = [species.ultra_item]
    else
  ...
  def drawPageData
    ...
    #---------------------------------------------------------------------------
    mega   = species_data.mega_stone || species_data.mega_move
    primal = species_data.form_name == _INTL("Primal {1}", species_data.name)
    ultra  = PluginManager.installed?("ZUD Mechanics") && species_data.ultra_item
    ...
    if mega || primal || ultra || gmax
      ...
      if gmax
        imagepos.push(["Plugins/ZUD Mechanics/Battle/icon_dynamax", 258, 45])
        check_form = @form
      elsif ultra
        imagepos.push(["Plugins/ZUD Mechanics/Battle/icon_ultra", 255, 45])
        check_form = species_data.ultra_base_form
      elsif primal
        imagepos.push(["Graphics/UI/Battle/icon_primal_#{species_data.name}", 256, 47])
        check_form = 0
      else
        imagepos.push(["Graphics/UI/Battle/icon_mega", 266, 53])
        check_form = species_data.unmega_form
      end
